...
undoinspect(PG_FUNCTION_ARGS)
{
	...
	if (!empty)
	{
		...
		record_ptr = UndoGetPrevUndoRecptr(insert, InvalidBuffer, category);
...
#define PG_STAT_GET_UNDO_RECORDS_COLS 5
			...
			{
				appendStringInfoString(&sb, sb.len > 0 ? ",T" : "T");
				if (IsXactApplyProgressCompleted(uur->uur_progress))
					appendStringInfoString(&sb, sb.len > 0 ? ",AC" : "AC");
				else if (!IsXactApplyProgressNotStarted(uur->uur_progress))
