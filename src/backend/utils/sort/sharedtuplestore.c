 ...
 * array there is a SharedBufFileSet.  This macro is needed to locate it,
 * because SharedBufFileSet also contains a final flexible array so can't be
 * declared as a regular member of SharedTuplestore.
 */
#define GetSharedBufFileSet(sts) \
	((SharedBufFileSet *) &(sts)->participants[(sts)->nparticipants])
		   ...
		   dsm_segment *segment)
{
	...
	SharedBufFileSetAttach(fileset, segment);
			 ...
			 void *meta_data, MinimalTuple tuple)
{
	...
	if (file == NULL)
	{
		...
		file = SharedBufFileCreate(fileset, partition, accessor->participant);
