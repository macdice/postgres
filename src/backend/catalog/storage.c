...
RelationTruncate(Relation rel, BlockNumber nblocks)
{
	 ...
	 * disk files. We need all of that work to complete before we release our
	 * lock, or else old versions of pages on disk that are missing recent
	 * changes would become accessible.  We'll try the whole operation again
	 * in crash recovery, but even then we can't give up because we don't want
	 * standbys' relation sizes to diverge and break other invariants.  The
	 * critical section also suppresses interrupts.
	 *
