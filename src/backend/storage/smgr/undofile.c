...
bool
undofile_can_open(RelFileNode rnode)
{
	return rnode.dbNode == UndoLogDatabaseOid;
}
					   ...
					   File file)
{
	...
	INIT_FILETAG(tag, reln->smgr_rnode.node, forknum, segno);

	/* Temp relations should never be fsync'd */
	Assert(!SmgrIsTemp(reln));

	if (!RegisterSyncRequest(&tag, SYNC_REQUEST, SYNC_HANDLER_UNDO,
							 false /*retryOnError*/))
	{
		ereport(DEBUG1,
