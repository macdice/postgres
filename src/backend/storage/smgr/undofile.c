...
undofile_sync(void)
{
	...
	while ((entry = (PendingOperationEntry *) hash_seq_search(&hstat)) != NULL)
	{
		...
		while ((segno = bms_next_member(requests, segno)) >= 0)
		{
			...
			/*
			 * The file may be gone due to concurrent discard.  It's also
			 * possible that we open a segment file that is subsequently
			 * recycled (renamed to represent a new range of undo log), in
			 * which case we'll fsync that instead.  That is rare and
			 * harmless.
			 */
			if (file <= 0)
				continue;
