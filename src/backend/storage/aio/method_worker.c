...
static int
pgaio_worker_choose_idle_worker(void)
{
	int			worker;
...
pgaio_worker_submit_internal(int num_staged_ios, PgAioHandle **staged_ios)
{
	...
	for (int i = 0; i < num_staged_ios; ++i)
	{
		...
		if (wakeup == NULL)
		{
			...
			worker = pgaio_worker_choose_idle_worker();
			if (worker >= 0)
	...
	{
		/*
		 * Between operations, try again to enqueue as many remaining
...
IoWorkerMain(const void *startup_data, size_t startup_data_len)
{
	...
	while (!ShutdownRequestPending)
	{
		...
		else
		{
			...
			{
				if ((worker = pgaio_worker_choose_idle_worker()) < 0)
					break;
