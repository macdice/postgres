...
{
	return offsetof(car_control, freelist) + sizeof(int) * objects;
}
...
car_allocate(car_control *car, car_mapping *mapping, car_mapping **replace)
{
	...
	if (car->need_cache_directory_replacement)
	{
		 ...
		 * Rx into Bx.  Now we have to decide whether to prefer B1 or B2.
		 * Unlike the paper, we also have to consider the R lists.
		 */
		car->need_cache_directory_replacement = false;
		if (mapping->list != B1 &&
			mapping->list != B2)
...
car_complete_replace(car_control *car, car_mapping *replace, bool accepted)
{
	...
	if (!accepted)
	{
		 ...
		 * soon.  This logic does not exist in the paper.  XXX May contain nuts
		 */
