...
{
	return 1024 * 1024 * (size_t) dynamic_shared_memory_main_size;
}
...
dsm_shmem_init(void)
{
	...
	dsm_main_space_begin = ShmemInitStruct("DSM Main Space", size, &found);
	if (!found)
	{
...
{
	/*
	 * We need to create a handle that doesn't collide with any
	 * existing extra segment created by dsm_impl_op(), so we'll
	 * make it odd.  It also mustn't collide with any other main
	 * area pseudo-segment, so we'll include the slot number in
	 * some of the bits.  We also want to make an effort to avoid
	 * newly created and recently destroyed handles from being
