...
#elif defined(HAVE_SYS_POLLSET_H)
...
struct WaitEventSet
{
	...
	struct pollfd  *pollset_ret_events;
...
#elif defined(WAIT_USE_POLLSET)
	set->pollset_ret_events = (struct pollfd *) data;
...
#elif defined(WAIT_USE_POLLSET)
	WaitEventAdjustPollset(set, event, PS_MOD);
#elif defined(WAIT_USE_POLL)
	WaitEventAdjustPoll(set, event);
#elif defined(WAIT_USE_WIN32)
	WaitEventAdjustWin32(set, event);
...
#if defined(WAIT_USE_POLLSET)
/*
 * action can be one of PS_ADD | PS_MOD.
 */
static void
WaitEventAdjustPollset(WaitEventSet *set, WaitEvent *event, int action)
{
	...
	ctl.cmd = action;
	ctl.events = events;
	rc = pollset_ctl(set->pollset_fd, &ctl, 1);
					  ...
					  WaitEvent *occurred_events, int nevents)
{
	...
	int			rc;
	WaitEvent  *cur_event;
	struct pollfd *cur_pollset_event;

	/* Sleep */
	rc = pollset_poll(set->pollset_fd, set->pollset_ret_events,
					  Min(nevents, set->nevents_space),
					  cur_timeout);
	...
	{
		/*
		 * If we used AIX 7.3's new extended pollset API, we could store a
		 * pointer to our WaitEvent object in pollfd_ext's data member as we do
		 * for kqueue and epoll.  But for compatibility with AIX 7.2, we have
		 * to search our array of WaitEvent objects for the matching fd.
		 */
		cur_event = NULL;
		for (int i = 0; i < set->nevents; ++i)
		{
			if (set->events[i].fd == cur_pollset_event->fd)
			{
				cur_event = &set->events[i];
				break;
			}
		}
		if (cur_event == NULL)
			ereport(ERROR,
					(errcode_for_socket_access(),
					 errmsg("pollset_poll() returned event for unknown fd %d",
							(int) cur_pollset_event->fd)));
		...
		if (cur_event->events == WL_LATCH_SET &&
			cur_pollset_event->revents & (POLLIN | POLLERR | POLLHUP))
		{
			/* Drain the self-pipe. */
			drain();
									  ...
									  WL_SOCKET_WRITEABLE))
		{
			...
			if ((cur_event->events & WL_SOCKET_WRITEABLE) &&
				(cur_pollset_event->revents & (POLLOUT | POLLERR | POLLHUP)))
			{
				/* writable, or EOF */
				occurred_events->events |= WL_SOCKET_WRITEABLE;
			}
