...
static bool ProcessBarrierCatchup(void);
	...
	if (flags != 0)
	{
		...
		PG_TRY();
		{
			...
			while (flags != 0)
			{
				...
				switch (type)
				{
					case PROCSIGNAL_BARRIER_CATCHUP:
						processed = ProcessBarrierCatchup();
						break;
				}
