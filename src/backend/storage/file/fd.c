...
check_io_direct(char **newval, void **extra, GucSource source)
{
	 ...
	 * alignment size, which could result in invalid I/O sizes for direct I/O.
	 * It might work if the true sector size is much smaller than our
	 * assumption, but that's unlikely on modern systems and can be overridden
	 * by also changing PG_IO_ALIGN_SIZE if appropriate.
	 */
