...
static void XLogPrefetcherReleaseBlock(uintptr_t pgsr_private,
									   uintptr_t read_private);
...
/*
 * A PgStreamingRead callback that examines the next block reference in the
 * WAL, and possibly starts an IO.  XXX talk about pins
 *
 * Returns PGSR_AGAIN if no more WAL data is available yet.
 *
 * Returns PGSR_NEXT_IO if the next block reference is for a main fork block
 * that isn't in the buffer pool, and the AIO system has been asked to start
 * reading it.
 *
						...
						uintptr_t *read_private)
{
	...
	for (;;)
	{
		...
		while (prefetcher->next_block_id <= record->max_block_id)
		{
			 ...
			 * XLogPrefetcherRead() (XXX check this is the right function), to
			 * assert that prefetching is in sync with replay.
			 */
			*read_private = (uintptr_t) block;
			...
			for (int i = 0; i < XLOGPREFETCHER_SEQ_WINDOW_SIZE; ++i)
			{
				...
				{
					/*
					 * XXX If we also remembered if we already had it pinned,
					 * we could bump the local pin count and include the
					 * buffer, so that recovery could skip smgropen() and a
					 * buffer table lookup.
					 */
			...
			/* Look up, and start IO if necessary.  Either way, the buffer is pinned. */
			block->prefetch_buffer =
				ReadBufferAsyncWithoutRelcache(reln, block->forknum, block->blkno,
...
/*
 * PgStreamingRead callback to make sure we unpin anything that was pinned by
...
XLogPrefetcherReadRecord(XLogPrefetcher *prefetcher, char **errmsg)
{
	...
	if (unlikely(XLogPrefetchReconfigureCount != prefetcher->reconfigure_count))
	{
		...
		prefetcher->streaming_read =
			pg_streaming_read_alloc(max_distance,
