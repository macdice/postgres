...
PadPartialPage(XLogRecPtr upto, XLogRecPtr *final_pad)
{
	...
	if (padded)
	{
		...
		WalStats.m_wal_partial_pad++;
		WalStats.m_wal_partial_pad_bytes += pad_size;
...
XLogWriteComplete(PgAioInProgress *aio, uint32 write_no)
{
	...
	if (flush_increased_to != InvalidXLogRecPtr)
	{
		...
		for (; start_segno < end_segno; start_segno++)
			XLogArchiveNotifySeg(start_segno);
	}
}
...
xlogwrite_again:
		...
		write_pos.flush_done_min <= LogwrtResult.FlushDone)
	{
		...
		if (!statted)
		{
			WalStats.m_wal_already_done_unlocked++;
			statted = true;
		}
		return performed_io;
	}
	...
	if (!LWLockAcquireEx(WALWriteLock, LW_EXCLUSIVE, XLogWriteLockCheck, write_pos.write_init_min))
	{
		tried_lock = true;
		holding_lock = false;
		WalStats.m_wal_lock_wait++;
		goto xlogwrite_again;
	}
	else
	{
		WalStats.m_wal_lock_immed++;
		holding_lock = true;
	}
		...
		write_pos.flush_done_min <= LogwrtResult.FlushDone)
	{
		...
		WalStats.m_wal_already_done_locked++;
		...
		if (lastSourceFailed)
		{
			...
			switch (currentSource)
			{
				...
				case XLOG_FROM_PG_WAL:
					...
					XLogPrefetchReconfigure(); /* TM:XXX why? */
					break;
