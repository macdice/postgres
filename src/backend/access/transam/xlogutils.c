							  ...
							  Buffer *buf)
{
	...
	if (XLogRecBlockImageApply(record, block_id))
	{
		...
		{
			/*
			 * The prefetcher has pinned this buffer, but used
			 * PGSR_NEXT_WILL_ZERO when it saw that an image would be
			 * restored, so the buffer wasn't made BM_VALID.  The only thing
			 * we are allowed to do now is call ZeroBuffer().  Doing this at
			 * the last moment means that we only lock one block at a time,
			 * when the redo routine is ready for it, and can tell us which
			 * kind of lock it wants.
			 *
