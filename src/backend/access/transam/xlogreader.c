...
XLogReleasePreviousRecord(XLogReaderState *state)
{
	...
	else
	{
		...
		else if (state->decoding && !state->decoding->oversized)
		{
			 ...
			 * XLogReadRecord(), but some time earlier we patially decoded a
			 * record in XLogReadAhead() when we hit an error.  We'll set the
			 * buffer head and tail to point to the record we started working
			 * on, so that we can continue (perhaps from a different source).
			 */
		...
		else
		{
			 ...
			 * start of the buffer space, because we're empty.
			 */
...
XLogReadRecord(XLogReaderState *state, XLogRecord **record, char **errormsg)
{
	...
	for (;;)
	{
		...
		if (state->decode_queue_tail)
		{
			 ...
			 * benefit of all the existing redo handler code that accesses
...
XLogDecodeOneRecord(XLogReaderState *state, bool allow_oversized)
{
	...
	switch (state->readRecordState)
	{
		...
		case XLREAD_TOT_LEN:
		{
			...
			{
				/*
				 * We couldn't get space.  If allow_oversized was true, then
				 * palloc() must have failed.  Otherwise, report that our
				 * decoding buffer is full.  This means that weare trying to
				 * read too far ahead.
				 */
				if (allow_oversized)
					goto err;
