...
BeginInsertUndo(UndoPackContext *ucontext, UnpackedUndoRecord *uur)
{
	...
	{
		ucontext->urec_txn.urec_progress = uur->uur_txn->urec_progress;
		ucontext->urec_txn.urec_dbid = uur->uur_txn->urec_dbid;
		ucontext->urec_txn.urec_next = uur->uur_txn->urec_next;
	}
	...
	{
		ucontext->urec_blkprev = uur->uur_prevundo;
	}

	/* Copy undo record log switch header if it is present. */
	if ((uur->uur_info & UREC_INFO_LOGSWITCH) != 0)
	{
		ucontext->urec_logswitch.urec_prevurp =
			uur->uur_logswitch->urec_prevurp;
		ucontext->urec_logswitch.urec_prevlogstart =
			uur->uur_logswitch->urec_prevlogstart;
	}
...
{
	/* Copy undo record header and common information. */
	...
	if ((uur->uur_info & UREC_INFO_TRANSACTION) != 0)
	{
		uur->uur_txn = palloc(sizeof(UndoRecordTransaction));
		uur->uur_txn->urec_progress = ucontext->urec_txn.urec_progress;
		uur->uur_txn->urec_dbid = ucontext->urec_txn.urec_dbid;
		uur->uur_txn->urec_next = ucontext->urec_txn.urec_next;
	}

	/* Copy undo record block prev header if it is present. */
	if ((uur->uur_info & UREC_INFO_BLKPREV) != 0)
	{
		uur->uur_prevundo = ucontext->urec_blkprev;
	}

	/* Copy undo record log switch header if it is present. */
	if ((uur->uur_info & UREC_INFO_LOGSWITCH) != 0)
	{
		uur->uur_logswitch = palloc(sizeof(UndoRecordLogSwitch));
		uur->uur_logswitch->urec_prevurp =
			ucontext->urec_logswitch.urec_prevurp;
		uur->uur_logswitch->urec_prevlogstart =
			ucontext->urec_logswitch.urec_prevlogstart;
