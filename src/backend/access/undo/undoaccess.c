...
{{.valid = false,.last_urecptr = InvalidUndoRecPtr},
{.valid = false,.last_urecptr = InvalidUndoRecPtr},
{.valid = false,.last_urecptr = InvalidUndoRecPtr},
{.valid = false,.last_urecptr = InvalidUndoRecPtr}};
...
#define UNDO_PAGE_PARTIAL_REC_SIZE(phdr)	\
	UndoRecordHeaderSize((phdr)->uur_info) + (phdr)->tuple_len +	\
						(phdr)->payload_len - (phdr)->record_offset;
/*
				  ...
				  Buffer buffer)
{
	...
	else
	{
		 ...
		 */
		offset = SizeOfUndoPageHeaderData + UNDO_PAGE_PARTIAL_REC_SIZE(phdr);
				  ...
				  Oid dbid)
{
	 ...
	 * allocating enough space for a transaction header.  Similarly low switch
	...
	if (need_xact_header)
	{
		urec->uur_txn = palloc(sizeof(UndoRecordTransaction));
...
{
	/*
	 * If any of the common header is not available in the current undo record
	 * then we must read it from the first complete record of the page.
	 */
	if (((ucontext->urec_hd.urec_info & UREC_INFO_RMID) == 0) ||
		((ucontext->urec_hd.urec_info & UREC_INFO_RELOID) == 0) ||
		((ucontext->urec_hd.urec_info & UREC_INFO_XID) == 0) ||
		((ucontext->urec_hd.urec_info & UREC_INFO_CID) == 0))
	{
		...
		if (undo_phdr->record_offset != 0)
			partial_rec_size += UNDO_PAGE_PARTIAL_REC_SIZE(undo_phdr);
