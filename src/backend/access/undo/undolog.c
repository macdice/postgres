...
LogNumberFromXid(TransactionId xid)
{
	if (InRecovery)
	{
		...
		return MyUndoLogState.xidToLogNumber[high_bits][low_bits];
	}
	else
	{
		int			i;
		...
		/*
		 * TODO: you can be attached to multiple logs, and we don't know which
		 * one to return so we'll have to grovel through and find the one
		 * this xid is associated with.  Is that OK or is it telling us that
		...
		for (i = 0; i < UndoPersistenceLevels; ++i)
		{
			...
			if (log && log->meta.xid == xid)
				return log->logno;
		}
	}

	elog(ERROR, "cannot find undo log for xid %u", xid);
}
...
attach_undo_log(UndoPersistence persistence, Oid tablespace)
{
	...
	if (log == NULL)
	{
		 ...
		 */
		log->checkpoint_in_progress =
			MyUndoLogState.shared->checkpoint_in_progress;
...
choose_undo_tablespace(bool force_detach, Oid *tablespace)
{
	...
	else
	{
		 ...
		 * useful.
		 */
		const char *name = list_nth(namelist, MyProcPid % length);
		Oid		oid;
...
DropUndoLogsInTablespace(Oid tablespace)
{
	...
	for (logno = low_logno; logno < high_logno; ++logno)
	{
		...
		if (log->meta.discard == log->meta.insert &&
			(log->meta.xid == InvalidTransactionId ||
			 !TransactionIdIsInProgress(log->meta.xid)))
		{
			prepare_to_modify_undo_log(log);
			log->meta.xid = InvalidTransactionId;
			if (log->pid != InvalidPid)
...
#define PG_STAT_GET_UNDO_LOGS_COLS 9
	...
	for (logno = low_logno; logno < high_logno; ++logno)
	{
		...
		if (log->meta.xid == InvalidTransactionId)
			...
			values[6] = TransactionIdGetDatum(log->meta.xid);
		if (log->pid == InvalidPid)
			nulls[7] = true;
		else
