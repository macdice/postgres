 ...
 * UndoLogControl objects are arranged in a fixed-size array, at arbitrary
 * positions.
 */
...
UndoLogDiscard(UndoRecPtr discard_point, TransactionId xid)
{
	...
	if (segno < new_segno)
	{
		...
		while (pointer < new_segno * UndoLogSegmentSize)
		{
			...
			/*
			 * Before removing the file, make sure that sync.c knows that it
			 * might be missing.
			 */
			INIT_FILETAG2(tag,
						  UndoLogDatabaseOid,
			...
			RegisterSyncRequest(&tag, SYNC_FORGET_REQUEST, SYNC_HANDLER_UNDO, true);
...
undolog_xlog_discard(XLogReaderState *record)
{
	...
	while (old_segment_begin < new_segment_begin)
	{
		...
		/*
		 * Before removing the file, make sure that sync.c knows that it might
		 * be missing.
		 */
		INIT_FILETAG2(tag,
					  UndoLogDatabaseOid,
