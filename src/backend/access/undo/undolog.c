...
static void scan_undo_log_physical_range(void);
 ...
 */
static void
undo_log_adjust_physical_range(UndoLogNumber logno,
							   UndoLogOffset new_discard,
							   UndoLogOffset new_insert)
{
	 ...
	 * recovery.  TODO:TM or you hold discard_lock?
	 */
	Assert(InRecovery ||
		   CurrentSession->attached_undo_slots[slot->meta.category] == slot);

	/* Therefore the slot can't have been recycled after we looked it up. */
	Assert(slot->logno == logno);
				...
				UndoRecPtr *prevlog_xact_start)
{
 ...
 retry:
	...
	/* Do we need more physical space? */
	if (unlikely(new_insert > end))
	{
		...
		undo_log_adjust_physical_range(slot->logno, 0, new_insert);
	}
						  ...
						  UndoRecPtr *prevlog_xact_start)
{
	...
	if (context->try_location != InvalidUndoRecPtr)
	{
		 ...
		 */
		slot = find_undo_log_slot(logno, false);
		if (UndoLogOffsetPlusUsableBytes(try_offset, size) <= slot->end)
		{
...
UndoLogDiscard(UndoRecPtr discard_point, TransactionId xid)
{
	...
	/*
	 * Log the discard operation in the WAL first before updating anything in
	 * shared memory.
	 */
	if (!InRecovery &&
	 ...
	 * non-advanced begin, discard and end points.  That's OK; we'll
	 * eventually try to discard again.
	 */
	LWLockAcquire(&slot->meta_lock, LW_EXCLUSIVE);
 ...
 */
static void
scan_undo_log_physical_range(void)
{
	...
	for (int i = 0; i < ntablespaces; ++i)
	{
		...
		while ((de = ReadDirExtended(dir, tablespace_path, LOG)))
		{
			...
			if (!slot)
			{
				 ...
				 * be left-over junk, but for now we don't try to figure that
				 * out.
				 */
				continue;
			}
			 ...
			 * exist we'll tolerate that.
			 */
			elog(LOG, "*** scanned %s", de->d_name);
