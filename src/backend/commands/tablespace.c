...
DropTableSpace(DropTableSpaceStmt *stmt)
{
	...
	if (!destroy_tablespace_directories(tablespaceoid, false))
	{
		 ...
		 * global barrier to ask all backends to process the shared
		 * invalidation queue, and wait until they're finished.
		 */
#if defined(WIN32) || defined(USE_ASSERT_CHECKING)
		WaitForProcSignalBarrier(EmitProcSignalBarrier(PROCSIGNAL_BARRIER_CATCHUP));
#endif
		/* And now try again. */
		if (!destroy_tablespace_directories(tablespaceoid, false))
		{
