 ...
 */
static int
libpqrcv_receive(WalReceiverConn *conn, char **buffer, pg_stream **wait_stream)
{
	int			rawlen;
	...
	if (rawlen == 0)
	{
		...
		if (rawlen == 0)
		{
			...
			*wait_stream = PQstream(conn->streamConn);
			return 0;
		}
	}
