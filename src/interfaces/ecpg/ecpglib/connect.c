...
static mtx_t connections_mutex;
static tss_t actual_connection_key;
static once_flag actual_connection_key_once = ONCE_FLAG_INIT;
...
{
	mtx_init(&connections_mutex, mtx_plain);
	tss_create(&actual_connection_key, NULL);
}

void
ecpg_pthreads_init(void)
{
	call_once(&actual_connection_key_once, ecpg_actual_connection_init);
}
...
ecpg_get_connection_nr(const char *connection_name)
{
	...
	if ((connection_name == NULL) || (strcmp(connection_name, "CURRENT") == 0))
	{
		...
		ret = tss_get(actual_connection_key);
...
ecpg_get_connection(const char *connection_name)
{
	...
	{
		mtx_lock(&connections_mutex);

		ret = ecpg_get_connection_nr(connection_name);

		mtx_unlock(&connections_mutex);
	}
...
ecpg_finish(struct connection *act)
{
	if (act != NULL)
	{
		...
		if (tss_get(actual_connection_key) == act)
			tss_set(actual_connection_key, all_connections);
...
ECPGsetconn(int lineno, const char *connection_name)
{
	...
	tss_set(actual_connection_key, con);
	return true;
}
