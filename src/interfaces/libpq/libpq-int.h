...
struct pg_conn
{
	...
	/* Support for external IO system calls */
	bool		io_external;
	PQIO		io;
...
/* === in fe-io.c === */
extern int	pqio_recv(PGconn *, void *ptr, size_t len, int flags);
extern int	pqio_send(PGconn *, const void *ptr, size_t len, int flags);
extern int	pqio_connect(PGconn *, const struct sockaddr *name, socklen_t namelen);
