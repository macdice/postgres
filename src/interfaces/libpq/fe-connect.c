...
connectNoDelay(PGconn *conn)
{
	...
	if (setsockopt(pg_socket_descriptor(conn->sock), IPPROTO_TCP, TCP_NODELAY,
				   (char *) &on,
				   sizeof(on)) < 0)
	{
		...
		case CONNECTION_NEEDED:
			{
				 ...
				 */
				{
					...
					if (pg_socket_set_blocking(conn->sock, false) != 0)
					{
						appendPQExpBuffer(&conn->errorMessage,
...
#ifdef F_SETFD
					if (fcntl(pg_socket_descriptor(conn->sock),
...
#ifndef WIN32
						else if (setsockopt(pg_socket_descriptor(conn->sock),
					 ...
					 */
					if (connect(pg_socket_descriptor(conn->sock),
								addr_cur->ai_addr,
		...
		case CONNECTION_STARTED:
			{
				...
				if (getsockopt(pg_socket_descriptor(conn->sock),
							   SOL_SOCKET, SO_ERROR,
				...
				if (getsockname(pg_socket_descriptor(conn->sock),
		...
		case CONNECTION_MADE:
			{
					...
					conn->raddr.addr.ss_family == AF_UNIX)
				{
					...
					errno = 0;
					if (getpeereid(pg_socket_descriptor(conn->sock),
								   &uid, &gid) != 0)
					{
						/*
...
sendTerminateConn(PGconn *conn)
{
	 ...
	 */
	if (conn->sock && conn->status == CONNECTION_OK)
	{
		/*
...
PQsocket(const PGconn *conn)
{
	if (!conn)
		return -1;
	return conn->sock ? pg_socket_descriptor(conn->sock) : -1;
}

/*
 * Return a socket object suitable for use in the backend.  This function is
 * not intended for use by client code and may change.
 */
Socket *
PQbackendsocket(PGconn *conn)
{
	return conn->sock;
}
