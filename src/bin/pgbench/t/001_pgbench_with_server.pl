...
TODO:
{
	...
	# The rate may results in an unlucky schedule which triggers an early exit,
	# hence the loose bound.  May also fail on very slow or overloaded hosts,
	# valgrind runs, etc, so this check is marked as TODO (expected to fail).

	ok(1.5 < $delay && $delay < 2.5, "-T 2 run around 2 seconds");
}

# $nthreads threads, 2 seconds.  We expect 1-3 log lines per thread, but due to
# timing imprecision we might have arbitarily many more on very slow systems.
# Tolerate up to 180.
#
...
check_pgbench_logs($bdir, '001_pgbench_log_1', $nthreads, 1, 180,
