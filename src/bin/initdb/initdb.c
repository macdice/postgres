...
/*
 * Save a copy of the current global locale name, for the given category.  The
 * returned value must be passed to restore_global_locale().
 *
 * Since names from the environment haven't been vetted for non-ASCII
 * characters, we need to use the wchar_t variant of setlocale() on Windows.
 * Otherwise they might not survive a save-restore round trip: when restoring,
 * the name itself might be interpreted with a different encoding by plain
 * setlocale() after we switch to another locale.  (This is a problem only in
