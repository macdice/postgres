...
undo_allocate(PG_FUNCTION_ARGS)
{
	...
	bool need_xact_header;		/* thrown away for now */
	UndoPersistence persistence = undo_persistence_from_text(PG_GETARG_TEXT_PP(1));
	UndoRecPtr undo_ptr;

	undo_ptr = UndoLogAllocate(size, InvalidUndoRecPtr, persistence,
							   &need_xact_header);
...
undo_append_file(PG_FUNCTION_ARGS)
{
	...
	start_undo_ptr = UndoLogAllocate(size, InvalidUndoRecPtr, persistence,
									 &need_xact_header);
