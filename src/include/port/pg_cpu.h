 ...
 * 0-N.  The 'int' format computed below is probably not very future proof.
 */
typedef PROCESSOR_NUMBER pg_cpu_t;
#define PG_CPU_FORMAT "%d:%d"
#define PG_CPU_FORMAT_PRINTF_ARG(x) (x).Group, (x).Number
#define PG_CPU_NUMBER(x) ((x).Number)
#define PG_CPU_GROUP(x) ((x).Group)
#define PG_CPU_AS_INT(x) (((x).Group * 64) + (x).Number)
#define PG_CPU_FROM_INT(x) ((pg_cpu_t){(x) / 64 + (x) % 64})
