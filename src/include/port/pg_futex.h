				  ...
				  struct timespec *timeout)
{
	 ...
	 * If false, remove link and fail with EAGAIN.  If so, sleep on proc
	 * latch. The main complication is that it wouldn't work for DSM segments;
	 * for those, we could have variants that take a dsm_segment and pointer
	 * and convert that to segment key + offset.
	 */
	errno = ENOSYS;
#endif
