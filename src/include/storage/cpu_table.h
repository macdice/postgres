...
#define CPU_TABLE_MAX_CPU_SETS		16
...
typedef struct cpu_table
{
	...
	int			cpu_set_to_numa_node[CPU_TABLE_MAX_CPU_SETS];
	int			map_shift;
...
extern PGDLLIMPORT int cpu_table_build(cpu_table *table,
									   int cpu_sets_per_numa_node);
extern PGDLLIMPORT int cpu_table_num_cpu_sets(const cpu_table *table);
extern PGDLLIMPORT int cpu_table_get_numa_node(const cpu_table *table,
											   int cpu_set_number);
extern PGDLLIMPORT int cpu_table_get_cpu_set_for_cpu(const cpu_table *table,
													 pg_cpu_t cpu);
extern PGDLLIMPORT bool cpu_table_can_run_on_cpu_set(const cpu_table *table,
													 int cpu_set_number);
extern PGDLLIMPORT void cpu_table_run_on_cpu_set(const cpu_table *table,
												 int cpu_set_number);
...
static inline int
cpu_table_current_cpu_set(const cpu_table *table)
{
